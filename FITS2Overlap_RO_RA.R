...
Otu = read.table("FITS2Otu_ssendo.txt", header = T , check.names = F , stringsAsFactors = F)
taxa <- read.table("FITS2Taxa_ssendo.txt" , header = T , check.names = F , stringsAsFactors = F) # mapping file containes informatio about data, lab part
sam_otu1 = Otu
...
agglot = aggregate(sam_otu_lotRAMelt1[,3], list(sam_otu_lotRAMelt1$OTU, sam_otu_lotRAMelt1$Species), sum)
colnames(agglot) = c("OTU","Species1","sumAbundance")
agglot$Species2 = paste(agglot$OTU,agglot$Species1, sep=" ")
agglot$Rank_Lotus_RA = rank(-agglot$sumAbundance, ties.method= "first") 
agglot25 = subset(agglot, agglot$Rank_Lotus_RA<26)


overlaplot_merge = merge(x =RO_nomatch , y = agglot25 , by.x  = "OTU" , by.y = "OTU")
overlaplot_merge1 = overlaplot_merge[c(1,2,3,18,4,17,16,20,15)]
...
overlapara_merge = merge(x = RO_nomatch1 , y = aggara25 , by.x  = "OTU" , by.y = "OTU")
overlapara_merge1 = overlapara_merge[c(1,2,3,20,4,18,17,22,16)]


#####---Write-tables---#####


write.table(overlaplot_merge1,file = "FungiLotusSpeciesTOP25_RA_RO.txt",quote = FALSE,sep = "\t",row.names = FALSE)
write.table(overlapara_merge1,file = "FungiArabidopsisTOP25_RA_RO.txt",quote = FALSE,sep = "\t",row.names = FALSE)
...
for (i in 1:2){
  var = variable[1]
  b <- ggplot()  + geom_bar(data=overlaplot_merge1, aes_string(x=OTUspe, y=var, fill = OTUspe), stat="identity") +
    ...
    scale_x_discrete(limit = overlaplot_merge1$OTUspe, labels = overlaplot_merge1$Rank_Lotus_RO) + 
    ...
    xlab("Rank Lotus") +
    ggsave(paste("Overlapping_RO_RA_FungiSpecies",variable[1],".png" , sep = "") , b , width = 25, height = 15, units = "in")
...
for (i in 1:2){
  var = variable[2]
  b <- ggplot()  + geom_bar(data=overlapara_merge1, aes_string(x=OTUspe, y=var, fill = OTUspe), stat="identity") +
    #facet_wrap(.~ Plant, scales = 'free_x' , nrow = 1 , strip.position="bottom")  + 
    scale_fill_manual(values=rev(col3)) +
    scale_y_continuous(labels = scale,limits = c(0,1)) + 
    scale_x_discrete(limit = overlapara_merge1$OTUspe, labels = overlapara_merge1$Rank_Arabidopsis_RO) + 
    guides(fill=guide_legend(ncol=1))  + 
    theme_classic() + theme(text = element_text(size=30 , colour = "black"),
                            axis.text = element_text(size = 30 , colour = "black" ),
                            axis.title = element_text(size = 30 , colour = "black" ),
    ...
    ggsave(paste("Overlapping_RO_RA_FungiSpecies",variable[2],".png" , sep = "") , b , width = 25, height = 15, units = "in")
  #ggsave(paste("MatchingROBacteriaSpecies",variable[1],".png" , sep = "") , a , width = 45 , height = 12 , units = "in")
}
